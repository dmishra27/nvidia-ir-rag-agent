...
- **Confirm condition (1) — weak partial support.** Of the two queries with a genuine terminology mismatch, Q4 improved (+3 fused ranks) and R1-Q7 was unchanged — the rewrite reaches only the dense query, which already ranks the target first; the failure is downstream in fusion, where rewriting has no lever. 1 of 2 improved, 0 worsened. **Q4 is also the only query in the full 16 where the rewrite moves the target's dense rank at all** (16 → 10); every other dense rank is identical baseline-to-gated. Round 2's *nominal* Case 4 (Q10, Q11) has no terminology mismatch and the rewriter correctly no-ops on both. *(DEF-28 caveat: Q4's target is ungraded per ENH-11 — doesn't change this reading; see DEF-28.)*
...
*DEF-28 caveat: this table's fixed target chunks are the same 16-anchor set checked against
`enh11_qrels.json` in DEF-28 (`docs/uat/correction_notice_a1.md` §6). Q3, Q5 and Q13's targets
grade only 1 ("topically related, doesn't answer"), and Q4's target is ungraded (absent from its
own query's ENH-11 pool). This doesn't change any rank/delta figure above — see DEF-28 for detail.*
...
*DEF-28 caveat: Q4's target chunk is ungraded — absent from its own query's ENH-11 pool, never
independently judged — per DEF-28 (`docs/uat/correction_notice_a1.md` §6). This doesn't change the
rank movement measured above.*
...
*DEF-28 caveat: Q5's target chunk grades only 1 ("topically related, doesn't answer") per
`enh11_qrels.json` — doesn't change this reading; see DEF-28 (`docs/uat/correction_notice_a1.md` §6).*
...
- **DEF-28 caveat.** Q4's target is ungraded and Q5's grades only 1 per `enh11_qrels.json` — both already-known weak/edge cases; this doesn't change the "do not ship" recommendation. See `docs/uat/correction_notice_a1.md` §6 (DEF-28).
