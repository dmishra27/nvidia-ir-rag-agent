...
DEF-28 caveat (added 13 September 2026): Q3's and Q5's target chunks grade only 1 in
`enh11_qrels.json`, not independently verified as strong answers — doesn't change the rank
recoveries above; see DEF-28, `docs/uat/correction_notice_a1.md` §6.*
...
DEF-28 caveat: Q5 — one of the two queries this confirmed verdict fires on — has a target chunk
that grades only 1 in `enh11_qrels.json`, not independently verified as a strong answer. The rank
recovery itself (17 → 2, dense's own rank) is unaffected — B7's claim is about rank position, not
answer quality — but see DEF-28 (`docs/uat/correction_notice_a1.md` §6) before citing Q5 as one of
the "two queries where fusion fails hardest."*
